...
const slider = function (slidenumber=0) {
  ...
  let all_vehicles_page = document.querySelector(".all-vehicles-page")

  
   const goToSlide = function (slide) {
    let vehicle_page = document.querySelector(".vehicle_slider")
    console.log(vehicle_page);

    if(!single_vehicle_page && !all_vehicles_page){
    if (!vehicle_page) {
      console.log(document.getElementById(`slide-no-${slide}`));
        ...
        slides.forEach(
          (s, i) => {
            (s.style.transform = `translateX(${50 * (i - slide)}%) scale(0.88)`);
              ...
              // if(s.querySelector(".card-img-top")) {
...
const sendMessage = function (e) {
  ...
  const fd = new FormData()
  fd.append('csrfmiddlewaretoken', csrf[0].value)
  fd.append("received_by", sender_id)
  fd.append("message_text", inputValue)
  console.log("------");
  console.log(fd.get("csrfmiddlewaretoken"));
  console.log(fd.get("received_by"));
  console.log(fd.get("message_text"));
